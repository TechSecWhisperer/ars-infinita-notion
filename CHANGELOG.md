...
- **🎯 Active Quests** — a table view on the Quest Board: active quests only, sorted by next action due. The answer to "what am I playing right now" without asking.
- **📈 XP: last 30 days** — a table view on the XP Ledger: the last month of raw XP, newest first. Your streak and level are derived numbers the agent reports; the ledger is the raw truth, and seeing it is what makes progress feel real rather than asserted.
- **🎖️ Earned** — a gallery view on Achievements showing **only what you have actually unlocked**. This is deliberately narrower than the database: a view persists, refreshes itself, and is visible to anyone the page is shared with, so it can never enforce the never-reveal rule turn by turn the way your agent can. Locked tiers, future badges and sealed mechanics stay invisible here — permanently, by design. The next-tier hint stays where it belongs: in your agent's reply, under the existing rule.

Each view is created idempotently (checked by name, never duplicated) with a plain-language fallback telling you how to add it yourself if your surface can't create views — the same pattern as the calendar view. Migration runs now create any of the four that are missing, and the migration inventory verifies all four by name — the gap that left one migrated instance without its calendar (issue #27) would otherwise repeat for the three new views.

**Note on numbering:** mechanics version moves to 2.0.13 in lockstep. Copy/delivery change only — no XP value, level threshold, badge criterion or unlock rule moved — Patch.
...
**Note on numbering:** mechanics version moves to 2.0.11 in lockstep. Copy/delivery change only — Patch.
...
It targets the optional layer only — mandatory Career quests track real applications and are not mercy-able. If a mercy has already been used this calendar month, yours is still honoured; it is just noted in the log. The monthly note exists to surface patterns, never to gate the valve. And if the game is wearing you out more broadly, quiet mode is still there.

Also in this release: a one-word grammar fix in the `/forge` cover-letter steps.

**Note on numbering:** mechanics version moves to 2.0.8 in lockstep. A new command is a minor-semantics addition, but it changes no XP value, level threshold, badge criterion or unlock rule, and it ships alongside delivery-only copy — Patch.
...
**It also adapts to where the job is, not where you are.** Resume convention genuinely differs by country, and getting it wrong costs you in both directions: two pages is right in the UK and over-long in the US, a photo is conventional in Germany and can get an application discarded in Australia or the US, and some European employers expect a data-processing consent line. `/armor` reads the target country from the quest and applies that market's conventions for length, spelling, dates, personal details, referees and work rights — then tells you in one line what it changed so you can overrule it. Adapting to a market changes how your history is presented, never the facts in it.

**Under the hood:** the guidance is authored once in `resume-library.json` and rendered to the reference the skill reads, with the sync enforced by `test/checks.mjs` so the two cannot drift apart. Each rule carries a provenance tag, and the validator refuses to mark a rule "verified" against a source it has no record of having read.

**Note on numbering:** the mechanics version moves to 1.3.10 in lockstep, as it does on every release. No XP value, level threshold, badge criterion or unlock rule changed, and nothing is re-scored.
