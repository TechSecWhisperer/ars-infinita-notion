...
| **`@ars-infinita-notion/system-skills`** | `packages/system-skills/package.json` | The skills as a Codex or Antigravity user receives them — the package ships them prebuilt, so this is game content, not just tooling. Tracks the plugin version exactly; `release-metadata-check` fails the build if the two disagree. |
