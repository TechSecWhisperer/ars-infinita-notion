...
which any agent that can make an HTTP request or run `curl` can do. There is no browser-gated check here — never mark one "needs a browser" or "needs desktop". The only honest SKIP is a capability the session genuinely lacks (e.g. no scheduled-task tools for the Daily Quest Briefing sub-check, or no HTTP capability at all for the mirror — in which case the Notion Patch Feed still answers the same question). Never present a partial run as a clean bill of health.
