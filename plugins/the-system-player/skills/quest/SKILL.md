...
2. **Fetch the posting** — hand the URL to `/browse` and name what you need. Route selection (clean session vs the player's own authenticated browser), bot-detection and checkpoints are `/browse`'s to handle, not yours; do not run a fetch ladder of your own here. Extract: role title, company, location, employment type, salary (if listed — most postings omit it, that's normal, don't guess a number), and the full job description text.

3. **Find or create the Gate Intel company row** (data source `KERNEL:Gate Intel`). Check for an existing row before creating a new one — don't create a duplicate company. Use a narrow probe, not a full-view pull: `notion-query-data-sources` with `SELECT * FROM "<resolved Gate Intel collection:// URL>" WHERE Company LIKE ? LIMIT 1`, binding the company name as a parameter. This scales with the 0-or-1 answer you need, not with the number of companies tracked. Caveats: `ILIKE` is **not** supported in SQLite and will error; `LIKE` is case-insensitive for ASCII only (so "NESTLÉ" won't match "Nestlé"), and treats `%` and `_` as wildcards, so escape them in company names. If SQL mode returns `entitlement_required` (free-plan quota), fall back to `notion-query-database-view` on the default view per the boot card. If new, create it with just the basics (Company, Industry, HQ/Locations, Website if known, `Research Status: Not Started`) — deep research is /scout's job, not /quest's.
...
3. **Compute where it stands against the SLA clock** the same way /grind would for this one quest: business days since `Last Activity` (the player's timezone, per the Kernel) — flag 🟠 if a follow-up is due (2+ business days) **only when the quest is at `Recruiter Screen` or later**, 🔴 if it's gone quiet (14+ days). Pre-contact quests (`Saved`, `Applied`) get no overdue flag at all — report elapsed time as a plain number if it's useful, never as a deadline.

4. **Suggest the natural next command** based on `Stage` and what's missing, e.g.: `Saved` + no Fit Score → "not yet appraised, want me to run /appraise?"; `Saved` + appraised but no Current Cover Letter note and they're leaning toward applying → "want me to /forge the cover letter?"; `Saved`/`Applied` + time elapsed → optionally "want to nudge them?", phrased as an option and never as an overdue item; `Recruiter Screen`/`Hiring Manager`/`Panel` upcoming and no Raid Prep note → "want a /raid playbook before this one?"; `Closed – No Response` → mention /respawn is available if they want another shot. Offer, don't just do — Continue mode is a briefing, not an excuse to auto-run another command.
