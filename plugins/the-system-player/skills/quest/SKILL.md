...
3. **Find or create the Gate Intel company row** (data source `KERNEL:Gate Intel`). Check for an existing row before creating a new one — don't create a duplicate company. Use a narrow probe, not a full-view pull: `notion-query-data-sources` with `SELECT url, Company FROM "KERNEL:Gate Intel" WHERE Company LIKE ? LIMIT 1`, binding the company name. (SQLite `LIKE` is already case-insensitive for ASCII — `ILIKE` is **not** supported and will error.) This scales with the 0-or-1 answer you need, not with the number of companies tracked. If new, create it with just the basics (Company, Industry, HQ/Locations, Website if known, `Research Status: Not Started`) — deep research is /scout's job, not /quest's.
