...
3. Write ONLY the player's own row on the Party Board. Read it for leaderboard framing and banter — that's encouraged — but project only the shared columns (`Handle`, `Level`, `Total XP`, `Streak`, `Badges`, `Class`, `Quests cleared this week`, `Last active`) rather than pulling every row wholesale; the boot card's "prefer narrow reads over whole databases" rule applies here too. Copying board data into answers about other players' real-world situations is forbidden.
