...
6. **Export send-ready files from the approved master:**
   ...
   - **.docx** — **probe first, and degrade honestly.** A `docx` skill is available in some sessions and ships with nothing here, so it may simply not exist where you are running. If one is present, read its SKILL.md and build with it. If it is not, say so plainly — *"no .docx builder is available in this session; here is the HTML, which opens in Word and can be saved as .docx from there"* — and deliver the HTML. **Never fall back to an arbitrary document library to fill the gap:** a player hit exactly that (public #64), got a `.docx` Word refused to open, and had to repair the OOXML by hand. A named missing capability is a good outcome; a corrupt file the player discovers at submission time is not.
