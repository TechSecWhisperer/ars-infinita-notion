...
7. **Forge Roulette** — first forge for this quest only (step 2 said so), and only after the letter is filed. It is a small delight, not a balance lever, and it holds no secret: the values live here in the open (ruling C, 2026-07-31) — there is nothing to fetch from anywhere else, and nothing to conceal.
   ...
   - **Roll with real randomness** (hard rule 11): `bash -c 'echo $((RANDOM % 3))'` — a result of `0` is a drop (a 1-in-3 chance). Never "pick" a number; if no random source is available this session, skip the roll and log an Open Question (the roll, not the letter, waits).
   - **On a drop:** log **+15 XP** to the XP Ledger — `Action` = "Forge Roulette drop: <role> @ <company>", `XP` = 15, `Related Quest` linked, `date:Date:start` = today (fetch the Ledger schema on first use; pick the closest existing `Category` option rather than inventing one). Recompute Total XP, check level thresholds, announce: `⚒️ [SYSTEM] The Forge hums — bonus XP granted (+15)`.
   - **On no drop (or cadence-skip):** say nothing about the roll at all — no "better luck next time", no acknowledgment that a roll happened. The delight is the occasional unexplained drop, not a hidden rule: don't narrate the machinery unprompted. Do not record no-drop outcomes anywhere player-visible; the once-per-quest key is the existence of any Cover Letter note for the quest (step 2), not a ledger row.
   - **If the player asks directly** — how the Forge works, the odds, the payout — answer plainly and completely (a 1-in-3 roll on a quest's first forge, +15 XP, at most 2 drops in any 7 days). Never deflect or feign ignorance: these values sit in this file, which the player is free to read. The quiet is about not spoiling a small surprise, never about keeping a secret from the person playing.
