...
**The player should never have to create a project folder by hand.** Creating one is a
machine's job: the folder is plumbing for the context files below, not something the
player needs to understand or name. The older version of this step said "attach a folder
and re-run" whenever the session had none attached — that outsourced a machine step to the
player, and it was the single largest piece of avoidable setup friction in /awaken.
...
2. **No folder attached, but the session can write files.** Propose a default in one line
   — `<home>/the-system` (e.g. `~/the-system`) — and ask **one** question: *"Where should
   your System project live? (Enter = ~/the-system)"*. On the player's yes-or-enter, create
   it with a plain `mkdir -p` and proceed. On a named path, use theirs. If they decline
   entirely (they want no folder on this machine), record that and go straight to the
   outcome-4 skip below — never create a folder they refused.
...
player has answered (or once question 2's default was accepted): it is empty, it is
