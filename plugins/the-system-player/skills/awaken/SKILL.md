...
- **The canonical view window — four views, one set.** Step 1 creates the game's whole window into the workspace, so the player can *see* their game without asking an agent to describe it. Each is a native view on its own database, created with the same idempotent check-by-name and the same one-line plain fallback as **📆 Next Actions** above, and nothing that exists is ever deleted or renamed:
  1. **📆 Next Actions** (⚔️ Quest Board, calendar, dated on `Next Action Due`, filtered to non-empty) — existing; leave it alone unless missing.
  2. **🎯 Active Quests** (⚔️ Quest Board, table) — filtered to active quests only (`Stage` not one of the terminal stages: 'Rejected', 'Closed – No Response', 'Withdrawn'), sorted `Next Action Due` ascending then `XP`-bearing rows first. The answer to "what am I playing right now" without asking.
  3. **📈 XP: last 30 days** (📊 XP Ledger, table) — filtered to `Date` within the last 30 days, sorted `Date` descending. Shows `Date`, `XP`, `Category`, `Action`. The streak and level are derived numbers the agent reports; the ledger is the raw truth, and seeing it is what makes progress feel real rather than asserted.
  4. **🎖️ Earned** (🏆 Achievements, gallery) — filtered to `Status` is '🏆 Unlocked' **only**. This is deliberately narrower than the database: a view persists and refreshes itself and is visible to anyone the page is shared with, so it can never enforce the never-reveal rule turn by turn the way the agent can. Locked tiers, future badges and sealed mechanics stay invisible here — the next-tier hint belongs in the agent's spoken reply under the existing rule, never in a permanent surface.
...
**Step 1's view creation is part of the migration inventory, unconditionally.** "Schema changes described in the patch" does not narrow it away: verify the Quest Board carries the **📆 Next Actions** calendar view (dated on `Next Action Due`), the **🎯 Active Quests** table, the **📈 XP: last 30 days** table on the XP Ledger, and the **🎖️ Earned** gallery on Achievements (earned rows only), and create any that are missing, exactly as Step 1 mandates on the build path — all four views, by name, not just the calendar. Legacy instances migrated without this check lose the calendar writers and never gain the replacement view (issue #27) — an inventory that checks only one of the four repeats that failure for the other three. And report the inventory honestly — state what was actually verified: entity presence **and** the view check. If views were not checked, write "views not checked"; never an unqualified "nothing was missing".
