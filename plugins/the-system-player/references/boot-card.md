...
- **agent-browser required** (needs a live browser session): `/scout`, `/quest` on a JS-heavy or LinkedIn posting, the web-research half of `/recruit` and `/gather`, and the stats-heartbeat push. Note two things that are **not** browser-gated: the **Patch Feed and Rule Manifest live in Notion**, reachable through the connector, so the boot head-check and the Sigil Check are Universal; and `/petition` (plus handle registration and party requests, which route through it) files a **GitHub Issue** via `gh issue create` — a plain shell call, with a paste-the-link fallback when `gh` is absent.
