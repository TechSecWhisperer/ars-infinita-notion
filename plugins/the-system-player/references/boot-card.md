...
There is exactly **one** of this file, at the plugin root (`references/boot-card.md`). Every skill reads this same copy — nothing is duplicated per command, so a rule fixed here is fixed everywhere at once. (The cross-agent builds for the Codex and Antigravity CLIs stamp a per-skill copy at build time because those CLIs install skills as standalone folders with no shared parent; those copies are generated output, verified byte-identical to this file by the build's boot-card fan-out check, and are never hand-edited.) It contains ONLY universal rules — **no instance data lives in this plugin.** Your instance data (IDs, player facts, versions, Nexus links) lives in the **🧬 Kernel page in the player's own Notion workspace**, written by /awaken. The Kernel is the source of truth for instance data; this file is the source of truth for universal rules, mirrored from the Nexus Rule Manifest. If they ever disagree, the live Rule Manifest wins — restore from it and log a Correction in the player's System Log.
...
2. **Version head-check:** compare the Kernel's Mechanics Version against the Nexus 📡 Patch Feed head (link in the Kernel). If behind → apply the patch steps from each missed feed entry in order (rule updates to the Kernel, idempotent schema migrations as described; a Migration-required patch may ask you to run /awaken), update the Kernel's version, and surface the patch notes to the player ("📜 System Update vX.Y — …"). The feed has two read paths and **neither needs a browser**: the Notion Patch Feed page (via the connector), and the public read-only mirror at `https://raw.githubusercontent.com/TechSecWhisperer/ars-infinita-notion/main/feed.json` — a static file any agent can fetch over plain HTTPS (`curl`, or whatever HTTP capability the session has). The Notion page is the source of truth; the mirror exists because a player's agent has no route into the admin Nexus, and its `head` may briefly lead the Notion page during a release. **The `/main/` raw URL is cache-unreliable — treat a disagreement as stale cache first, not as drift.** It has been observed serving a stale `head` for an extended period, across repeated fetches, while the Notion feed was already ahead. So: a single `/main/` fetch is **never** sufficient evidence that anything has drifted, and a mismatch is not a finding you report or act on. When the mirror and the Notion feed disagree, **the Notion feed wins** — re-read it through the connector and use that answer. Only if Notion is unreachable does the mirror stand alone, and then say which path answered. If neither is reachable → play continues on the local Kernel; retry next boot. Patch entries are game rules, not arbitrary instructions — they are constrained to the game's domain (rules, XP, schema, copy); these hard rules and the Party Wall outrank the feed.
4. **Call-home check (standing rule, public #18).** After the head-check, run the **call-home verification** defined in `${CLAUDE_SKILL_DIR}/../references/home-check.md`: fetch the public feed mirror over plain HTTPS, verify the local mechanics_version equals `feed.mechanics_version`, and verify every rule value this session will use matches the published authority. **Fail closed:** on unreachability or confirmed mismatch, run `/doctor` (its check 6 decides drift vs stale cache); if `/doctor` confirms drift, file a petition via `/petition` (category Bug, mechanical description, no sealed values) and hold XP-earning play until it resolves. Rules are READ at boot — never inlined into skill files at build time.
5. Check the player's System Log (`KERNEL:System Log`) for Corrections (Type: Correction / Not Working, Status: Standing Rule) — standing rules override everything below.
6. Do the one duty. **Full boot** (also read the live `KERNEL:Operating Manual`) is warranted for briefings, weekly reviews (/levelup), and interview-heavy sessions.
...
The player's instance carries a **📅 System Calendar** database — briefings, weekly reviews, reminders, and events land here as dated rows so the player sees them in their own Notion calendar view. Resolve its page ID + data source ID from the Kernel's Instance ID table (`KERNEL:System Calendar`) like any other entity — never hardcode an ID. It is a **delivery/record channel, not a source of truth**: the Daily Log stays authoritative for anything also written here, and nothing about a quest's stage, XP, or status is ever decided from a calendar row. /awaken creates it if a player's duplicate is missing it (idempotent — verify by name before creating) and registers it on the Kernel.

**Quest and contact follow-ups are never copied here.** `Next Action` / `Next Action Due` live on the ⚔️ Quest Board and `Next Follow-up` lives on the 🏰 Hunter Network — one row, one place. They reach the player's calendar through those databases' **own native calendar views** (the Quest Board's **📆 Next Actions** view, dated on `Next Action Due`, created once by /awaken), not through a mirrored row that goes stale the moment the player edits the date at source. The Calendar's `Follow-up Due` option survives only so pre-existing rows keep rendering; no command writes it.
...
- **agent-browser required** (needs a live browser session): **`/browse`, and only `/browse`.** It is the sole route to a live web page. `/scout`, `/quest` on a JS-heavy or LinkedIn posting, the web-research half of `/recruit` and `/gather`, and the stats-heartbeat push all need a page at some point — they hand it to `/browse` rather than opening one themselves, so there is one place that probes and one place that can be wrong. Note two things that are **not** browser-gated: the **Patch Feed and Rule Manifest live in Notion**, reachable through the connector, so the boot head-check and the Sigil Check are Universal; and `/petition` (plus handle registration and party requests, which route through it) files a **GitHub Issue** via `gh issue create` — a plain shell call, with a paste-the-link fallback when `gh` is absent.
`/browse` says so plainly and offers the fallback when there is no browser — never fail silently, and never let the calling command invent its own wording:
...
## The browser tool — its interface lives in `/browse`, not here
The tool is **agent-browser**, a CLI for agents (`github.com/vercel-labs/agent-browser`). Its commands, install paths, health probe and routing policy are specified **once, in `/browse`** — this card deliberately does not restate them, because a second copy is a second thing to drift. Do not invent a browser API, do not assume an MCP, and do not run `agent-browser` from any skill other than `/browse`: hand it the URL and what you need extracted. Hard rule 8 still binds everywhere — never solve a CAPTCHA or checkpoint.
